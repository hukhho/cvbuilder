...
import './ai.css';
import { format, parse, startOfMonth, isDate } from 'date-fns';
...
const CustomForm = ({
  ...
  const parseDate = (dateString, formatString) => {
    try {
      ...
      return isDate(parsedDate) ? parsedDate : new Date(); // Return parsed date or current date if parsing fails
    } catch (error) {
  ...
  useEffect(() => {
    if (experience) {
      ...
      try {
        ...
        console.log('startDateString', startDateString);
        const parsedStartDate = parseDate('February 2022', 'MMMM yyyy');
        console.log('parsedStartDate: ', parsedStartDate);

        const parsedEndDate =
          endDateString === 'Present' ? new Date() : parseDate(endDateString, 'MMMM yyyy');
      ...
      >
          ...
          label={
            <label className="!leading-[15px] label flex flex-col justify-between lg:flex-row lg:items-end text-xs uppercase text-gray-600">
              <div className="flex gap-2 items-center text-xs">
                <span>
                  WHAT WAS YOUR <strong>TITLE</strong> FOR THIS SECTION?{' '}
                </span>
              </div>
            </label>
          }
        ...
        <Form.Item
          ...
          label={
            <label className="!leading-[15px] label flex flex-col justify-between lg:flex-row lg:items-end text-xs uppercase text-gray-600">
              <div className="flex gap-2 items-center text-xs">
                <span>
                  FOR WHICH <strong>SUB TITLE</strong>? *
                </span>
              </div>
            </label>
          }
            ...
            <Form.Item
              ...
              label={
                <label className="!leading-[15px] label flex flex-col justify-between lg:flex-row lg:items-end text-xs uppercase text-gray-600">
                  <div className="flex gap-2 items-center text-xs">
                    <span>
                      <strong>WHERE</strong> WAS IT LOCATED?
                    </span>
                  </div>
                </label>
              }
        ...
        <div className="flex items-end justify-between">
          <label className="!leading-[15px] label flex flex-col justify-between lg:flex-row lg:items-end text-xs uppercase text-gray-600">
            <div className="flex gap-2 items-center text-xs">
              <span>
                <strong>WHAT DID YOU DO</strong> AT THE IT?
              </span>
            </div>
          </label>
          {/* <button
