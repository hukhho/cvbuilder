...
import '@/app/components/Form/customtext.css';
import React, { useEffect, useState } from 'react';
import { Col, ConfigProvider, Empty, Input, Row, Select, Typography } from 'antd';
...
const Home = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const filterJobByLiked = fetchedDataFromAPI.filter(job => job.liked === true);
      setFilteredData(filterJobByLiked);
    } catch (error) {
      console.error('Error fetching data:', error);
    }
  };

  const onLikeJob = async () => {
    fetchData();
  };
                  ...
                  <Row gutter={[16, 48]}>
                    {filteredData.map((job, index) => (
                      <Col key={index} span={12}>
                        <JobCard job={job} jobTitle={job?.title} onLikeJob={onLikeJob} />
                      </Col>
                    ))}
